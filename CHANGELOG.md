...
- Added a Transpiler-Mate plugin registered through the `transpiler_mate.plugins` entry point, with a required `--inputs` option for YAML workflow inputs.
- Added regression tests for explicit JSON schemas, CQL2 evaluation errors, and the required-property example.
- Added generated Python API reference documentation using MkDocstrings.

### Changed

- **Breaking:** moved command-line validation to `transpiler-mate assertions-mate`. Install `transpiler-mate-runtime` alongside `assertions-mate` to use the command; the Python library remains available.
- Updated examples with workflow metadata and requirements, and changed example tasks to run through Transpiler-Mate with the local project.
- Updated runtime dependencies to `cwl2ogc>=0.20.0`, `eoap-problems-registry>=1.5.0`, and `transpiler-mate-api==1.0.0`; relaxed version constraints for `pygeofilter`, `regopy`, and `requests`.
- Updated project documentation and repository URLs to the `Transpiler-Mate` organization.

### Removed

- Removed the standalone `assertions-mate` command, its `assertions_mate.cli` module, and the `cli` installation extra.
- Removed built-in `file://`, `s3://`, and `oci://` adapters from JSON Schema retrieval and the direct `session-adapters` dependency.

### Fixed

- Explicit JSON schemas, including an empty `{}` schema, now take precedence over schemas derived from the parent workflow during validation and serialization.
- CQL2 predicate evaluation failures now raise `Cql2EvaluationError` with the rule identifier and preserve the underlying exception; validation runtime errors are propagated as `PluginFailureError` by the plugin.
- Rego validation now reports every expression returned by a query and converts each error detail to a string.
...
[Unreleased]: https://github.com/Transpiler-Mate/assertions-mate/compare/v0.10.1...develop
[0.10.1]: https://github.com/Transpiler-Mate/assertions-mate/compare/v0.10.0...v0.10.1
[0.10.0]: https://github.com/Transpiler-Mate/assertions-mate/compare/v0.9.0...v0.10.0
[0.9.0]: https://github.com/Transpiler-Mate/assertions-mate/compare/v0.8.0...v0.9.0
[0.8.0]: https://github.com/Transpiler-Mate/assertions-mate/compare/v0.7.0...v0.8.0
[0.7.0]: https://github.com/Transpiler-Mate/assertions-mate/compare/v0.6.0...v0.7.0
[0.6.0]: https://github.com/Transpiler-Mate/assertions-mate/compare/v0.5.0...v0.6.0
[0.5.0]: https://github.com/Transpiler-Mate/assertions-mate/compare/v0.4.0...v0.5.0
[0.4.0]: https://github.com/Transpiler-Mate/assertions-mate/compare/v0.2.0...v0.4.0
[0.2.0]: https://github.com/Transpiler-Mate/assertions-mate/releases/tag/v0.2.0
